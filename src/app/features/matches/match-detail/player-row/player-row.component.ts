...
@Component({
  ...
  template: `
    <li class="player-row" [class.current]="isCurrent()">
      ...
      @if (canWithdraw()) {
        <button class="btn-withdraw" (click)="withdraw.emit(reg().player_id)" aria-label="Retirer de la liste">✕</button>
      }
      @if (canDelete()) {
        <button class="btn-delete" (click)="delete.emit(reg().player_id)" aria-label="Supprimer l’inscription">🗑</button>
      }
    </li>
  `,
  styles: `
    .player-row {
      ...
      padding: 0 0.75rem; border-radius: 0.4rem; transition: background 0.1s;
    }
    ...
    .name {
      flex: 1; min-width: 6ch; font-size: 0.95rem;
    ...
    .btn-withdraw {
      ...
      font-size: 0.8rem; opacity: 0.7; transition: opacity 0.15s;
    }
    ...
    .btn-delete {
      ...
      min-width: var(--tap); min-height: var(--tap); flex-shrink: 0;
      display: inline-flex; align-items: center; justify-content: center;
      font-size: 0.9rem; border-radius: 0.3rem;
    }
  `,
})
